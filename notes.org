* Contents :TOC:
 ...
 - [[#plans][Plans]]
     - [[#underway-use-grep-to-find-matching-lines][UNDERWAY Use grep to find matching lines]]
...
** UNDERWAY Use grep to find matching lines
:LOGBOOK:
-  State "UNDERWAY"   from "MAYBE"      [2017-04-12 Wed 17:26]
:END:
...
I think this might work pretty well:

1.  Run =egrep= with a regexp like =(^\*|keyword)=.  This will return all headings /and/ all lines that match the keyword.
2.  For each non-heading line in the output, look up one line to get its heading, then concat them and fontify them and return them as a candidate.  (Will need to get line-numbers, and will need to make the goto functions handle line numbers as well as positions.

** TODO [#B] Try org-map-entries
