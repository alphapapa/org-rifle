...
** UNDERWAY org-rifle refactor
:LOGBOOK:
-  State "UNDERWAY"   from              [2019-08-09 Fri 07:35]
:END:

[2019-08-09 Fri 07:37]  Copying subtrees from =org-rifle.org= file, cherry picked from branch =wip/org-rifle-old=, which is the obsolete WIP branch.

New plan is to refactor, split up the packages, and /then/ look at adding SQLite indexing separately.
...
** TODO [#A] Use cache with ~org-get-outline-path~

I'm not sure if we can, but if so, it should help performance.

** MAYBE Use [[http://www.lesbonscomptes.com/recoll/][recoll]] for indexing
:LOGBOOK:
