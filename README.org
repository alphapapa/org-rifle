...
In contrast with =org-occur= and similar commands, =org-rifle= is entry-based (i.e. a heading and all of its contents, not including subheadings), while =org-occur= is line-based.  So =org-occur= will show you entire lines that contain matching words, without any reference to the heading the line is under, while =org-rifle= will show the heading of the entry that matches, followed by context around each matching word in the entry.  In other words, =org-rifle= is sort of like Google, while =org-occur= is sort of like =grep=.
...
*Helm support*: [[https://github.com/emacs-helm/helm][Helm]] support is available in the package =helm-org-rifle=.  It is recommended, because Helm provides features that significantly improve search speed and convenience.
...
With =org-rifle-show-path= set to =t=, the whole path to each heading is shown:
...
Install [[https://github.com/magnars/dash.el][dash.el]], [[https://github.com/rejeep/f.el][f.el]], and [[https://github.com/magnars/s.el][s.el]].  Then require this package in your init file:
...
+  Select multiple entries in the Helm buffer to display selected entries in a persistent, read-only results buffer.
+  Save all results in a Helm buffer to a persistent results buffer by pressing =C-s= (like =helm-grep-save-results=).
...
+  Delete the result at point in =org-rifle= results buffers by pressing =d=.  This does not alter the source buffers but simply removes uninteresting results from view.
+  You can customize options in the =org-rifle= group.

* Changelog                                                      :noexport_1:

** 2.0-pre

The package has been renamed to =org-rifle=, and Helm support has been factored out into a separate package, =helm-org-rifle=.
