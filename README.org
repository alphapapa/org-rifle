...
* Installation                                            :noexport_children:
...
If multiple candidates are selected in the Helm commands, it would be nice to display them in an occur-style buffer.  It seems like it should be simple, but it seems difficult to get a list of marked candidates from Helm, because =helm-marked-candidates= only returns the candidates for the current selection's source, not for all marked candidates.  Surely there is a way...
