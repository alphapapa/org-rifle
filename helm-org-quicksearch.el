           ...
           for source = (helm-build-sync-source (buffer-name buffer)
                          :candidates (lambda ()
                                        (when (s-present? helm-pattern)
                                          (helm-org-quicksearch-get-candidates-in-buffer (helm-attr 'buffer) helm-pattern)))
                          :match 'identity
                          :delayed 0.5
                          :multiline t
                          :volatile t
...
(defun helm-org-quicksearch-get-candidates-in-buffer (buffer input)
...
POSITION is the position in BUFFER where the candidate heading begins."
  (let* ((input (split-string input " " t))
    ...
    (with-current-buffer buffer
      (save-excursion
        (goto-char (point-min))
        (while (and (< helm-org-quicksearch-result-count helm-org-quicksearch-max-results)
                                                       ...
                                                       `(and (repeat 0 ,helm-org-quicksearch-context-words
                                                                     (and (1+ (not space))
                                                                          (or (1+ space)
                                                                              word-boundary)))
                                                             (group (eval token))
                                                             (repeat 0 ,helm-org-quicksearch-context-words
...
(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "test.org") "pomegr blueberry")
(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "test.org") "green")
(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "test.org") "green blue")
(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "test.org") "pomegr")

(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "test.org") "helm food")
...
(helm-org-quicksearch-get-candidates-in-buffer (get-file-buffer "main.org") "emacs helm")
...
(let* ( (needles '("needle" "pin"))
  ...
  (cl-loop for needle in needles
           append (cl-loop for re = (rx-to-string `(and (repeat 1 ,helm-org-quicksearch-context-words (and (1+ (not space))
                                                                                                           (or (1+ space)
                                                                                                               word-boundary)))
                                                        (group (eval needle))
                                                        (repeat 1 ,helm-org-quicksearch-context-words (and (or word-boundary
                                                                                                               (1+ space))
                                                                                                           (1+ (not space))))))
